  ...
  },
  {
    ...
    icon: (
      <svg width="22" height="22" viewBox="0 0 24 24" fill="none" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        <circle cx="5" cy="12" r="1" fill="var(--text)"/><circle cx="12" cy="12" r="1" fill="var(--text)"/><circle cx="19" cy="12" r="1" fill="var(--text)"/>
      </svg>
    ),
  },
];
  ...
  if (isLoading) {
    return (
      <div style={{ position: "absolute", inset: 0, background: "var(--bg)", display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div style={{ width: 26, height: 26, border: "2px solid var(--text)", borderTopColor: "transparent", borderRadius: "50%", animation: "spin 0.7s linear infinite" }} />
      </div>
    );
  }
  if (!contact) return (
  ...
  return (
    <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", background: "var(--bg)", overflow: "hidden" }}>
      ...
      <div style={{ flex: 1, overflowY: "auto", background: "var(--bg-2)" }}>
        ...
        {/* Avatar + Name header */}
        <div style={{ background: "var(--bg)", paddingTop: 24, paddingBottom: 28, display: "flex", flexDirection: "column", alignItems: "center" }}>
          <div style={{
        ...
        {/* Action buttons */}
        <div style={{ background: "var(--bg)", marginTop: 1, padding: "18px 16px 22px" }}>
          <div style={{ display: "flex", justifyContent: "center", gap: 20 }}>
            {actionButtons.map((btn) => (
              <div key={btn.label} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 7 }}>
                ...
                <span style={{ fontSize: 12, color: "var(--text)", fontWeight: 500 }}>{btn.label}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        {/* Settings card */}
        <div style={{ margin: "18px 16px 0", background: "var(--bg-card)", borderRadius: 16, overflow: "hidden" }}>
          {settingsRows.map((row, i) => (
            <button
              ...
              style={{
                ...
                borderBottom: i < settingsRows.length - 1 ? `0.5px solid var(--sep)` : "none",
        ...
        {/* Block / Report */}
        <div style={{ margin: "18px 16px 40px", background: "var(--bg-card)", borderRadius: 16, overflow: "hidden" }}>
          <button style={{ width: "100%", padding: "14px 16px", background: "none", border: "none", borderBottom: "0.5px solid var(--sep)", cursor: "pointer", fontSize: 16, color: "#ff3b30", textAlign: "center" }}>
            Block {contact.name}
          </button>
